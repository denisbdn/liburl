...
/** Преобразовывает punycode-строку в utf-8 */
char* from_punycode_to_utf8_reent_(const char *input, size_t input_size, char *output, size_t output_size) {
	char *p = output, *end = output + output_size - 5;
	const char *in;
	if (!input)
		return NULL;
	const char *end_input = input + input_size;
	while (input && input < end_input) {
		if ((in = (const char *)memmem(input, end_input - input, "xn--", strlen("xn--")))) {
			punycode_uint puout[MAX_DOMAIN_NAME];
			size_t plen = sizeof (puout) / sizeof (*puout);
			if (in != input) {
				memcpy(p, input, in - input);
				p += in - input;
			}
			in += 4;
			const char *bound = (const char *)memmem(in, end_input - in, ".", strlen("."));
			if (punycode_decode((bound ? bound - in : (end_input - in)), in, &plen, puout, NULL) == punycode_success) {
				unsigned i;
				for (i = 0; i < plen && p < end; i++)
					p = append(puout[i], p);
				if (p >= end)
					return NULL;
			} else
...
/** Преобразовывает punycode-строку в utf-8 */
const char* from_punycode_to_utf8_(const char *input, size_t input_size) {
	static char out[MAX_DOMAIN_NAME * 6 + 1];
	return from_punycode_to_utf8_reent_(input, input_size, out, sizeof(out));
}

/** Преобразовывает punycode-строку в utf-8 */
char* from_punycode_to_utf8_reent(const char *input, char *output, size_t output_size) {
	char *p = output, *end = output + output_size - 5;
	const char *in;
	if (!input)
		return NULL;
	while (input && *input) {
		if ((in = strstr(input, "xn--"))) {
			punycode_uint puout[MAX_DOMAIN_NAME];
			size_t plen = sizeof (puout) / sizeof (*puout);
			if (in != input) {
				memcpy(p, input, in - input);
			...
			in += 4;
			const char *bound = strchr(in, '.');
			if (punycode_decode((bound ? (size_t)(bound - in) : strlen(in)), in, &plen, puout, NULL) == punycode_success) {
				unsigned i;
				for (i = 0; i < plen && p < end; i++)
					p = append(puout[i], p);
				if (p >= end)
...
/** Преобразовывает punycode-строку в utf-8 */
const char* from_punycode_to_utf8(const char *input) {
	static char out[MAX_DOMAIN_NAME * 6 + 1];
	return from_punycode_to_utf8_reent(input, out, sizeof(out));
}

char* from_utf8_to_punycode_reent_(const char *input, size_t input_size, char *output, size_t output_size) {
	char *p = output;
	size_t out_len;
	if (!input)
		return NULL;
	const char *end_input = input + input_size;
	while (input) {
		punycode_uint puin[MAX_DOMAIN_NAME];
		const char *bound = (const char *)memmem(input, end_input - input, ".", strlen("."));
		size_t i = 0;
		for (i = 0; i < MAX_DOMAIN_NAME && input != bound && input < end_input; ++i)
			puin[i] = next(&input);
		if (i == MAX_DOMAIN_NAME - 1)
			return NULL;
		if (i) {
			strcpy(p, "xn--");
			p += 4;
		}
		out_len = output_size - (p - output) - 1;
		if (punycode_encode(i, puin, NULL, &out_len, p) == punycode_success) {
			p += out_len;
			if (i + 1 == out_len && *(p - 1) == delimiter) {
				p = (char *) memmove(p - out_len - 4, p - out_len, out_len - 1);
				p += (out_len - 1);
			}
		} else
			return NULL;
...
/** Преобразовывает utf-8 в punycode-строку */
const char* from_utf8_to_punycode_(const char *input, size_t input_size) {
	static char out[MAX_DOMAIN_NAME * 6 + 1];
	return from_utf8_to_punycode_reent_(input, input_size, out, sizeof(out));
}

/** Преобразовывает utf-8 в punycode-строку */
char* from_utf8_to_punycode_reent(const char *input, char *output, size_t output_size) {
	char *p = output;
	size_t out_len;
	if (!input)
		return NULL;
	while (input) {
		punycode_uint puin[MAX_DOMAIN_NAME];
		const char *bound = strchr(input, '.');
		size_t i = 0;
		for (i = 0; i < MAX_DOMAIN_NAME && input != bound && *input; ++i)
			puin[i] = next(&input);
		if (i == MAX_DOMAIN_NAME - 1)
			return NULL;
		if (i) {
...
/** Преобразовывает utf-8 в punycode-строку */
const char* from_utf8_to_punycode(const char *input) {
	static char out[MAX_DOMAIN_NAME * 6 + 1];
	return from_utf8_to_punycode_reent(input, out, sizeof(out));
}
